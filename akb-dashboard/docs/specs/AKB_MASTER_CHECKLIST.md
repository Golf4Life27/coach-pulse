...
| 12.8 | INEVITABLE_Constitution_v3.docx + Make scenarios — manual `Buyer_Tx_Median` → `Buyer_Median` token cleanup | NOT STARTED | LOW | INSERTED 5/18 with the Phase 20.2 v1.3 amendment. Tx prefix was a Texas-only launch artifact; system is nationwide. **Vercel-side code has zero references** — only one stale Checklist row carried the old name (corrected in Commit H). Manual follow-up surfaces: (a) `INEVITABLE_Constitution_v3.docx` if the formula is documented there; (b) Make scenarios J (Verify_Agent_V1), G (Deal_Alert_Blast_V1), D (Deal_Creation_Bridge_V1) if any reference the field by NAME rather than field id. Code could not access Make scenario blueprints from this sprint (team id not in repo + Make MCP requires it). Low priority because the actual Airtable Investor_MAO formula uses Real_ARV_Median, not Buyer_Tx_Median — so the rename is semantic correctness, not a live bug. |
