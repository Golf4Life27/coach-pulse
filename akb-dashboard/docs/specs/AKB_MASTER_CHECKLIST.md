...
| # | Item | Status | Severity | Notes |
|---|------|--------|----------|-------|
| 9.1 | Shepherd panel (persistent Maverick presence on every page) | NOT STARTED | CRITICAL | Foundational UX element. (CODE 5/16: confirmed — zero matches for "shepherd"/"german shepherd" in components. New component required.) |
| 9.2 | Priority surface BroCards wired to Maverick aggregator | NOT STARTED | CRITICAL | Highest-leverage change. Reuse existing BroCard family in `components/cards/` (DDCard, DealCard, HoldButton, ResponseCard, StaleCard). New Priority surface aggregator subscribes to `maverick_load_state` output. |
| 9.3 | Named-agent vocabulary in code (`@agent: sentinel/crier/sentry/forge/scribe/scout/pulse/appraiser/ledger`) | **NEW INSERTION** | HIGH | (CODE 5/16: **INSERTED 5/16 BECAUSE** the named-agent roster is canonical in three Maverick specs + Continuity Layer Spec §6 but DOES NOT EXIST in code attribution. Current attribution uses operational tags: `maverick`, `phase4c`, `d3-cadence`, `agent-prior-counts`. Without unified attribution, the Daily UX Spec's "agent rooms" cannot map cleanly. Must precede 9.4.) |
...
| 9.5 | Severity tier visual treatment (Tier 0/1/2/3) | NOT STARTED | HIGH | Daily UX Spec §5 |
...
| 9.11 | Jarvis→Maverick rename of components | **NEW INSERTION** | MEDIUM | (CODE 5/16: **INSERTED 5/16 BECAUSE** `components/JarvisChat.tsx`, `JarvisFeed.tsx`, `JarvisGreeting.tsx` + 4 `app/api/jarvis-*/` routes are pre-Maverick naming. Per Code Briefing §9: "Jarvis is the old name being replaced." Rename pass with backwards-compatible re-exports to avoid breaking imports. Low risk, high coherence value. Schedule alongside 9.1 since the Shepherd panel supersedes JarvisGreeting's role.) |
