...
| 11.6 | Cron-burn safeguard on `/api/maverick/load-state` + MCP route | **DONE-PARTIAL (gates landed; source-of-burn unidentified)** | CRITICAL | INSERTED 5/17 BECAUSE 48hr unattended Sat-Sun window burned ~4.8M tokens / ~$15-30 of Anthropic credits via paired aggregator (1.5K) + synthesizer (8.2K) calls every ~6 min. (CODE 5/17 cron inventory: `vercel.json` contains 5 crons, all DAILY (`0 X * * *` once-per-day) — none hit `/api/maverick/load-state`. Vercel Hobby plan hard-caps crons at once-per-day per `AGENTS.md` (`0 */6 * * *` rejected at deploy on commit `2e0d054`), so a 6-min Vercel cron literally cannot exist in this repo. Daily crons left intact: `propose-actions` 9am, `scan-comms` 10am, `scan-replies` 11am, `warmup-sequence` 1pm, `recompute-agent-prior-counts` 8am — all part of working pipeline machinery, none caused the burn. **Source of 6-min cadence remains unidentified.** Surviving hypotheses: (a) external uptime monitor pinging load-state, (b) persistent claude.ai session or `/loop`-style keepalive, (c) backgrounded dashboard tab where the 90s `ShepherdPanel.tsx` setInterval got browser-throttled to ~6min cadence, (d) cold-start cache misses making the in-process aggregator cache ineffective. Code-level guards shipped Commit B.2: `MAVERICK_CRON_ENABLED` env var (default false) gates `auth.kind==="cron"` callers in both `/api/maverick/load-state` (returns 503 + `load_state_cron_gated` audit) and `/api/maverick/mcp` (returns 503 + `mcp_cron_gated` audit). Phase 14 breadcrumb shipped: `pulse_event:"non_user_synthesis"` console.warn fires only when fresh synthesis runs from a non-`dashboard_session` caller — Pulse data trail for later anomaly detection. Correct warmup pattern (if briefing freshness at idle matters): on-demand invalidation when a user loads the dashboard with TTL ≥30min, NOT a periodic cron. |
