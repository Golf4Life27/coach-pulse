...
| OFF - Nightly pipeline audit | (off) | Was a fresh-session routine with no connector tools, so it never actually ran; recreate from the Routines UI if wanted | nothing |
| OFF - Weekly buyers list | (off) | Same: never had tools; recreate from the Routines UI if wanted | nothing |
...
Routine prompts live in `akb-dashboard/docs/handoffs/routine-prompts/` and are the source of truth for what each routine is told to do.
