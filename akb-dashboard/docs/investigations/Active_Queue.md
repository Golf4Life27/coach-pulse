...
6. **[INV-006] Outreach_Status / Stage transition logic on signed contracts** — INVESTIGATION COMPLETE 2026-05-20 → `docs/investigations/Outreach_Status_Transition_Audit_v1.md` → pending operator decision on remediation. Q1 finding: 0 records table-wide have `Envelope_ID` populated (Phase 12.7 DocuSign provisioning is operator-external STOP) — same architecture-vulnerability shape as INV-004 but cannot ship a runtime guard (transition needs a handler, not a predicate). Q2: largest gap is contract-execution end of ladder (`Negotiating/Response Received → Offer Accepted` has zero automation; operator `accept` click is the only writer). Two adjacent gaps logged separately (Response Received → Negotiating; stale Negotiating → Dead). Q3: Option β (cron reconciler) architecturally cleanest — Option γ (Airtable native automation) rejected by Belt v1 §6 + Master Checklist Rule 10 (no Spine, no audit); Option α (in-process) unimplementable today (Phase 12.7-gated). Q4: forward-only auto-transition + Notes-audit-line idempotency for operator-override discipline. Findings support Option D (Hybrid: β now as no-op + α at Phase 12.7). Operator picks A/B/C/D/E in chat.
