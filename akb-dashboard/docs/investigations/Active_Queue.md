...
4. **[INV-004] Crier stale-contact false-positive on fresh contracts** — INVESTIGATION COMPLETE 2026-05-20 → `docs/investigations/Crier_Stale_Contract_Audit_v1.md` → pending operator decision on remediation. Q1 finding: strict false-positive count today is **0** (no records have Envelope_ID populated; DocuSign provisioning is Phase 12.7 STOP) — but ~33 of 43 active Negotiating/Response Received records are currently Crier-silence-flagged with no contract-state signal to distinguish them. Fix is **preventive infrastructure** — Option A's guard suppresses nothing today but kicks in the moment DocuSign provisioning lands and operator starts tracking envelopes. Q2: Listings_V1↔Deals linkage is unreliable (only indirect via Offer_Drafts; 35/43 records have no Offer_Drafts link; Deals table has only 3 historical records). Q3: same blind spot exists in Pulse `stale-data-drift` detector. Q4: deal-commentary.ts header confirms architectural intent — Crier deterministic rules, Pulse confidence-scores them later. Findings effectively reject Option C (defer to Pulse — Pulse has same blind spot today); support Option D (hybrid: A now, B path planned). Operator picks A/B/C/D in chat.
